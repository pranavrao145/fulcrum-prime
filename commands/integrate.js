...
module.exports = {
    ...
    execute(msg, args, con) {
        ...
        if (!args[0]) {
            msg.reply("incorrect syntax! Correct syntax: f!integrate {service_command} {mention/role}. See f!services for a full list of services Fulcrum offers.");
            return;
        }
        ...
        if (args[0] === "channelcount") {
            ...
            if (!args[1]) {
                msg.reply("incorrect syntax! Correct syntax: f!integrate channelcount {voice_channel_role}")
                return;
            }
            ...
            let guild = msg.guild;
            let voice_channel = msg.guild.channels.cache.find(r => r.name === roleFromMention.name && r.type === "voice");
            ...
            if (!args[1]) {
                msg.reply("incorrect syntax! Correct syntax: f!integrate membercount {voice_channel_role}")
                return;
            }
            ...
            let offline_role = guild.roles.cache.find(r => r.name.toLowerCase() === "offline");
            ...
            guild.channels.cache.filter(c => c.type === "voice").forEach(vc => {
                let vc_role = guild.roles.cache.find(r => r.name === vc.name);

                if (!vc_role) {
                    guild.roles.create({
                        data: {name: `${vc.name}`}
                    }).then(() => console.log(`Voice channel role for ${vc.name} created successfully`));
                }
            });

            msg.reply("voice channel roles set up or updated successfully. NOTE: if you have voice channels with the same names as text channels, voice channel roles may not work properly. Recommended to ensure this is not the case.")

        } else if (args[0] === "datechannel") {
            let roleFromMention = getRoleFromMention(msg, args[1]);

            if (!args[1]) {
                msg.reply("incorrect syntax! Correct syntax: f!integrate datechannel {voice_channel_role}")
                return;
            }
