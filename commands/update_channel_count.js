    ...
    execute(guild = null, msg = null, client, con) {
        if (msg !== null) {
            ...
            con.query(`SELECT *
                    ...
                    if (!vc) {
                        msg.reply("sorry, channel count channel has not been set up yet. An admin f!integrate channel {voice_channel_role} to set this feature up.")
                    } else {
                        let count = guild.channels.cache.filter(c => (c.type === "voice" || c.type === "text") && !c.deleted).size;
            ...
            con.query(`SELECT *
                ...
                if (vc) {
                    let count = guild.channels.cache.filter(c => (c.type === "voice" || c.type === "text") && !c.deleted).size;
                    vc.setName(`💬|Channel Count: ${count}`)
                        .then(() => console.log("Channel count updated successfully."));
                }
            })
